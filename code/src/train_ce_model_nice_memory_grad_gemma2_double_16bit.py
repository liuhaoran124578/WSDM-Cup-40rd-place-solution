...
### 一台机器8卡 每张卡都会执行这个函数 其中local_rank代表在这台机器上的卡号
    ...
    def evaluation_dist(model, eval_dataloader):
        ...
        print("loss score:", loss, "accuracy:", acc, "number:", len(predicts))
        return loss,acc
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.save_batch_steps and (global_step + 1) % args.save_batch_steps == 0:
                loss_mean,acc_mean = evaluation_dist(model, eval_dataloader)
                ...
                if torch.distributed.get_rank() == 0 or args.zero_stage == 3 or True:
                    ...
                    print_rank_0(
                        f"***** Evaluating Acc, Epoch {epoch + 1}/{args.num_train_epochs}---{global_step}/{total_steps}*****",
                        args.global_rank)
                    print_rank_0(f"acc: {acc_mean}", args.global_rank)
                ...
                if acc_mean > best_val_acc:
                    print_rank_0(
                        f"val_log----epoch:{epoch},batch:{global_step + 1},save model from {best_val_acc} to {acc_mean} !!!",
                        args.global_rank)
                    save_model(args, model, tokenizer, f"best_val_acc_model")
                    best_val_acc = acc_mean
                    no_improve_epoch_acc = 0
                else:
                    no_improve_epoch_acc += 1
                    print_rank_0(
                        f"val_log----epoch:{epoch},batch:{global_step + 1},no_improve_epoch_acc:{no_improve_epoch_acc},curr_val_acc {acc_mean} best_val_acc {best_val_acc} !!!",
                        args.global_rank)
